...
Scripts :
  ...
  scan-resolve [args...]   Resout un lot d'ISBN comme des scans (pivot + tomes + jaquettes/resumes),
                           tous en parallele (N users premium simules) -> JSON found/not-found
                           ex: ./scripts/dev.ps1 scan-resolve [test/fixtures/scanned-isbns.txt] [--out r.json]
...
switch ($Command) {
    ...
    'scan-resolve'   { Invoke-Compose exec app npx ts-node --transpile-only scripts/scan-resolve.ts @Rest }
